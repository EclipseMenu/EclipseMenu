...
class $modify(cocos2d::CCKeyboardDispatcher) {
    bool dispatchKeyboardMSG(cocos2d::enumKeyCodes key, bool down, bool idk) {
        if (down) {
            eclipse::keybinds::Manager::get()->registerKeyPress(eclipse::keybinds::convertCocosKey(key));
        } else if (!down) {
            eclipse::keybinds::Manager::get()->registerKeyRelease(eclipse::keybinds::convertCocosKey(key));
        }
