...
namespace eclipse::keybinds {
    Keys convertMouseKey(int button) {
        switch (button) {
            ...
            case GLFW_MOUSE_BUTTON_4: return Keys::MouseButton4;
            case GLFW_MOUSE_BUTTON_5: return Keys::MouseButton5;
    ...
    class $modify(MouseKeybindingsManagerCCEGLVHook, cocos2d::CCEGLView) {
        void onGLFWMouseCallBack(GLFWwindow* window, int button, int action, int mods) {
            ...
            if (action == GLFW_PRESS) manager->registerKeyPress(key);
            else if (action == GLFW_RELEASE) manager->registerKeyRelease(key);
        }
    };
}
