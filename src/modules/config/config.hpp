...
namespace eclipse::config {
    ...
    T get(std::string_view key, T const& defaultValue) {
        ...
        auto valueRes = res.unwrap().as<T>();
        if (!valueRes) return defaultValue;
        return std::move(valueRes).unwrap();
    }
    ...
    geode::Result<T> get(std::string_view key) {
        return getStorage()[key].as<T>().mapErr([&] -> std::string {
            return fmt::format("Failed to get key '{}'", key);
        });
    }
    ...
    bool is(std::string_view key) {
        ...
        if constexpr (std::is_same_v<T, std::string>) {
            return type == matjson::Type::String;
        } else if constexpr (std::is_same_v<T, bool>) {
            return type == matjson::Type::Bool;
        } else if constexpr (std::is_same_v<T, int>) {
            return type == matjson::Type::Number;
    ...
    void setIfEmpty(std::string_view key, T value) {
        if (!has(key)) set(key, std::move(value));
    }
    ...
    geode::Result<T> getTemp(std::string_view key) {
        return getTempStorage()[key].as<T>().mapErr([&] -> std::string {
            return fmt::format("Failed to get key '{}'", key);
        });
    }
